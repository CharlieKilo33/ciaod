...
void ReadRecordFromFileAndInsertIntoTable(HashTable& hash_table) {
  ...
  std::fstream file("C:/Users/misha/CLionProjects/ciaod/3/3_1.bin",
                    std::ios::binary | std::ios::in);
  if (!BinaryFile::FileIsOpen(file)) {
    return;
  }
...
// void DeleteRecordByKey(HashTable& hash_table, const BinaryFile& file, const int key) {
//   // Find the record in the hash table
//   int record_position = find_key(hash_table, std::to_string(key))->position;
//   if (record_position == -1) {
//     std::cerr << "Error: Record with key " << key
//               << " not found in the hash table." << std::endl;
//     return;
//   }
//
//   // Open the binary file for deletion
//   std::fstream file(binary_file_manager.getFileName(),
